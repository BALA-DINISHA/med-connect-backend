...
const createPharmacy = async (req, res) => {
  try {
    const { name, email, phone, address, city, licenseNumber, status } = req.body

    if (!name || !phone) {
      return res.status(400).json({ success: false, message: 'Name and phone are required' })
    }

    const pharmacy = await Pharmacy.create({ name, email, phone, address, city, licenseNumber, status })
